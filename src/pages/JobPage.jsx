// This the page where the user can see the job details.
import React, { useEffect, useState } from 'react';
import { useParams, Link } from 'react-router-dom'; // Route parameters
import { getaJob } from '../utils/api'; // Fetch job details
import { ToastContainer,toast } from 'react-toastify'; // Notification
import supabase from '../utils/supabase2'; // Supabase
import {
  MapPin, Building2, DollarSign, Clock, Calendar,ChevronLeft, Briefcase, AlertCircle, CheckCircle,ScrollText, Users, Target, Award} 
from 'lucide-react'; // Icons
import { useUser } from '@clerk/clerk-react'; // User context
...
export default function JobPage() {
  ...
  const [isSaved, setIsSaved] = useState(false); // To track if the job is saved

  // Saving the job
  const handleSaveJob = async () => {
    // Inserting the data in Saved Jobs table in supabase
    try {
      ...
      // Handles error in saving job
      if (error) {
        console.error("Error saving job:", error.message);
        toast.done("Already Saved")
      // Job Saved
      }else{
  ...
  useEffect(() => {
    ...
    const checkSavedStatus = async () => {
      // Checking the Saved Jobs table in supabase
      const { data, error } = await supabase
        .from("Saved Jobs")
        .select("id")
        .eq("user_id", userId) 
  ...
  useEffect(() => {
    ...
    // Checking the Applications table in supabase
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-800 text-white">
        <div className="max-w-7xl mx-auto px-4 py-12">
          <Link to="/jobs"className="inline-flex items-center text-white/90 hover:text-white mb-8 transition-colors duration-200">
            <ChevronLeft className="w-5 h-5 mr-2" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
              ...
              {/* Experience Level, Employment Type, and Career Level -Left Side */}
          ...
          {/* Salary Range and Apply Before - Right Side */}
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-8 sticky top-8">
              ...
              {/* Apply Button and Save Job button */}
              </div>
